	...
	const handleSubmit = async (values, setSubmitting) => {
		...
		try {
			...
			const url = await singleImageUpload(
				`images/Organizations/${_id}`,
				barImages.file
			);

			// create user
			const prom1 = await firebase
				.auth()
				...
				.then(async (res) => {
					if (res.user) {
						...
						await ref.doc(res?.user?.uid).set(data, { merge: true });
					}
				});
			...
			Promise.allSettled([prom1, prom4])
				.then((Args) => {
                        ...
                        <Grid item xs={12} md={6}>
                          ...
                          type="password"
										...
										>
											<FormControlLabel
												value="Organization"
												checked={values.OrderCollection === 'Organization'}
												control={<Checkbox defaultChecked />}
												label="Organization"
											/>
											<FormControlLabel
												value="Dolce Days"
												checked={values.OrderCollection === 'Dolce Days'}
												control={<Checkbox />}
												label="Dolce Days"
											/>
										</FormGroup>
										...
										<FormControlLabel
											checked={values.Branding === 'true'}
											control={<Checkbox />}
											label="Enable Dolce Days Branding"
										/>
										<Grid item xs={12}>
